...
void	render_3d_projection_wall(t_game_data *data, t_fov *fov)
{
	...
	while (i < data->ray)
	{
		...
		ray_distance = fov[i].distance * cos(fov[i].angle - data->player.rotation_angle);
		fov[i].wall_strip_height = (TILE_SIZE / ray_distance) * distance_projection_plane;

		start.x = fov[i].id * WALL_STRIP_WIDTH;
		start.y = 0;
		draw_straight_line(data, &fov[i], start, (WINDOW_HEIGHT / 2) - (fov[i].wall_strip_height / 2), CEILING);

		start.y = (WINDOW_HEIGHT / 2) - (fov[i].wall_strip_height / 2);
		draw_straight_line(data, &fov[i], start, fov[i].wall_strip_height, WALL);

		start.y += fov[i].wall_strip_height;
		if (start.y < WINDOW_HEIGHT)
			draw_straight_line(data, &fov[i], start, WINDOW_HEIGHT, FLOOR);
		i++;
	}
}
